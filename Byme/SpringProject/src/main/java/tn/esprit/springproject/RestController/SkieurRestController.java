...
public class SkieurRestController {
    ...
    @PostMapping("/AssignToCours/{numCours}")
    public Skieur AssignSkieurToCours(@RequestBody Skieur sk, @PathVariable("numCours") Long numCours)
    { return iSkieur.AddAndAsignToCours(sk, numCours);}

    @GetMapping("/retrieveSkiersBySubscriptionType/{typeabon}")
   public List<Skieur> retrieveSkiersBySubscriptionType( @PathVariable("typeabon") TypeAbonnement typeAbonnement){
        return iSkieur.retrieveSkiersBySubscriptionType(typeAbonnement);
    }
